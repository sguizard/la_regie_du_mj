...
4. **Tokens** — the left sidebar shows, below the maps, the **list of the scene's
   ...
   **+ type**, **+ init**, **+ DEF** (defence / AC) and **+ HP** add them in one click.
   ...
   Row 3 (only when at least one of the two is set): **ATK** (attack bonus, free text:
   `+4`, or `+4 / +6` for a multiattack) and **DM** (damage in dice notation — `2d6`, `1d8 + 3`), both editable in
